...
export class SpectrumDetailsComponent implements OnInit {
    ...
    downloadSpectrumData(spectrum: Spectrum) {
        const dataStr =
            'data:text/json;charset=utf-8,' + encodeURIComponent(JSON.stringify(JSON.parse(spectrum.spectrumSamples)))
        ...
        this.dataService.deleteSpectrum(id).subscribe({
            next: () => {
                this.notificationService.showNotification(
                    new AppNotification(`Spectrum with id ${id} deleted`, NotificationType.success)
                )
